...
public class Aoc1903 {
    ...
    public static void main(String[] args) {
        ...
        var firstWire = raw.split("\\n")[0].split(",");

        var firstPath = new LinkedList<Coord>();

        var x = 0;
        var y = 0;

        for (var com : firstWire) {
            System.out.println("first wire: " + com);
            var dir = com.charAt(0);

            var length = Long.parseLong(com.substring(1));

            if (dir == 'R') {
                for (var i = 0; i < length; i++) {
                    x++;
                    firstPath.add(new Coord(x, y));
                }
            } else if (dir == 'U') {
                for (var i = 0; i < length; i++) {
                    y++;
        ...
        var secondwire = raw.split("\\n")[1].split(",");

        x = 0;
        y = 0;
        var wirelength = 0L;

        var results = new HashMap<Coord, Long>();
        System.out.println("first done");

        for (var com : secondwire) {
            System.out.println("second wire: " + com);
            var dir = com.charAt(0);
            ...
            if (dir == 'R') {
                for (var i = 0; i < length; i++) {
                    wirelength++;
                    x++;
                    if (firstPath.contains(new Coord(x, y) )) {
                        results.put(new Coord(x, y), 1 + wirelength + firstPath.indexOf(new Coord(x, y)));
                    }
                }
            } else if (dir == 'U') {
                for (var i = 0; i < length; i++) {
        ...
        for (var aaa : results.entrySet()) {
            System.out.println("finding min!");
            min = Math.min(min, aaa.getValue());
        }
