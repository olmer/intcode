...
public class Aoc2416 extends AbstractAoc {
  record Node(int col, int row, Direction facing, long cost, List<Pair<Integer, Integer>> path) {}

  record NodeShort(int col, int row, Direction facing) {}
  ...
  long part1(String[] in) {
    ...
    while (!q.isEmpty()) {
      ...
      Direction[] canGo = Arrays.stream(Grid.CARDINAL).filter(d -> d != Grid.OPPOSITES.get(n.facing)).toArray(Direction[]::new);
  ...
  long part2(String[] in) {
    ...
    Map<Direction, Long>[][] costs = new HashMap[in.length][in[0].length()];
